...
const FilterSearch = () => {
  ...
  const fetchCategoryData = async (blockName, departmentName, categoryName) => {
    try {
      const response = await fetch(
        `https://miscbit-10.onrender.com/api/block/category/data/${blockName}/${departmentName}/${categoryName}`
      );
      if (!response.ok) {
  ...
  const convertJsonToExcel = () => {
    ...
    const categories = ['Department', 'Labs', 'Classrooms', 'SeminarHalls', 'Washrooms'];
  ...
  const convertJsonToPDF = () => {
    ...
    const drawTable = (title, data, keys) => {
      ...
      doc.line((doc.internal.pageSize.getWidth() - titleWidth) / 2, 42, (doc.internal.pageSize.getWidth() + titleWidth) / 2, 42);
  ...
  return (
    <div className="container">
      <Grid item xs={12}>
        <Paper style={{ padding: '20px', background: '#fff' }}>
          <Grid container justifyContent="space-between" alignItems="center">
            <Grid item>
              <Typography variant="h1" sx={{ color: '#880000' }}>
                ALL DATA FILTER
              </Typography>
            </Grid>
          </Grid>
      ...
      <div style={{ textAlign: 'center', marginTop: '20px' }}>
        <Typography variant="h3" style={{ marginBottom: '30px', color: '#880000', fontWeight: 'bold' }}>
      ...
      {selectedBlock && Object.keys(blockData).length > 0 && !selectedDepartment && (
        <div>
          {(!selectedTable || selectedTable === 'Classrooms') && (
            <div>
              <Typography variant="h3" style={{ marginBottom: '20px', color: '#880000', fontWeight: 'bold' }}>
                CLASSROOMS
              </Typography>
          ...
          {(!selectedTable || selectedTable === 'SeminarHalls') && (
            <div>
              <Typography variant="h3" style={{ marginBottom: '20px', marginTop: '20px', color: '#880000', fontWeight: 'bold' }}>
                SEMINAR HALLS
              </Typography>
              <TableContainer component={Paper} style={{ maxHeight: 440 }}>
                <Table stickyHeader aria-label="sticky table">
