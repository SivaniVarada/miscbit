...
const FilterSearchblock = ({ block, department }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.data == null) {
        response.data = `NO ${selectedCategory} are there `
      }
      setDepartmentData(response.data);
      fetchJsonData(url);
    } catch (error) {
      console.error(`Error fetching ${selectedCategory} for department ${department} in block ${block}:`, error);
    }
  };
  ...
  style={{ width: '80%', marginBottom: '10px', borderRadius: '4px' }}
  displayEmpty
...
<Button
  ...
  style={{
    width: '10%',
    marginLeft: '20px',
    backgroundColor: '#BA2C1B', // maroon color
    color: 'white',
    borderRadius: '4px',
    height: '50px', // Adjust height to match Select component
  }}
  variant="contained"
>
  Generate
